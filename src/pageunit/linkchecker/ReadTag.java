...
public class ReadTag {
	...
	public List<Element> readTags() throws IOException {
		List<Element> foundTags = new ArrayList<Element>();
		Element aTag;
		while ((aTag = nextTag()) != null) {
			...
			if (wantedTags == null) {
				...
				String thisTagName = aTag.getType();
				for (String wantedTagName : wantedTags) {
					if (thisTagName.equalsIgnoreCase(wantedTagName)) {
						foundTags.add(aTag);
						break;
					}
				}
	...
	private void readAttributes(Element tag, int i) throws IOException {
		final int S_INNAME = -1, S_EQUALS = '=', Q_SQUOTE = '\'', Q_DQUOTE = '"'; /* Q_NONE = 'N',  */
